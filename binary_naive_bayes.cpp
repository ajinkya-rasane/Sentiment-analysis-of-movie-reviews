...
float pred_acc(string filename)// function to calculate probabilities of all documents
{
    ...
    while(getline(infile, line))
    {
        ...
        while(iss >> word)
        {
            ...
            pos_prob += calc_prob(word_id, 0);
            neg_prob += calc_prob(word_id, 1);
        }
        int pred;
...
{
    total_words = 89527;
    create_word_freq("labeledBow.feat");
    float accuracy = pred_acc("test_labeledBow.feat");
